...
class ft_net_LPN(nn.Module):
    ...
    def __init__(self, class_num, droprate=0.5, stride=2, init_model=None, pool='avg', block=6):
        ...
        # self.cbam2=CBAMBlock(channel=64)
...
class ft_net_LPN_convnext(nn.Module):

    def __init__(self, class_num, droprate=0.5, stride=2, init_model=None, pool='avg', block=6):
        super(ft_net_LPN_convnext, self).__init__()
...
class three_view_net_convnext(nn.Module):
    def __init__(self, class_num, droprate, stride = 2, pool = 'avg', share_weight = False, VGG16=False, LPN=False, block=6):
        ...
        if VGG16:
            ...
            self.model_1 =  ft_net_LPN_convnext(class_num, stride = stride, pool = pool, block = block)
            self.model_2 = ft_net_LPN_convnext(class_num, stride = stride, pool = pool, block = block)
        ...
        if share_weight:
            ...
            if VGG16:
                ...
                self.model_3 =  ft_net_LPN_convnext(class_num, stride = stride, pool = pool, block = block)
            else:
