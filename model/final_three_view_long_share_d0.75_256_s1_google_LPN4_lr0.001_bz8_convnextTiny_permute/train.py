...
import time
import os
from model import three_view_net_convnext, two_view_net, three_view_net
...
if opt.views == 2:
    ...
    if opt.LPN:
        model = three_view_net_convnext(len(class_names), droprate = opt.droprate, stride = opt.stride, pool = opt.pool, share_weight = opt.share, LPN = True, block = opt.block)
    else:
